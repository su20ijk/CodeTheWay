...
function sortDate(table, column) {
    var table, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0;
...
function search(table, toSearch) {
    var table = document.getElementById("myTable").rows;
    for (var i = 1; i < table.length; i++) {
        var containsSearch = false;
        var tblCells = table[i].cells;
        for (var j = 0; j < tblCells.length; j++) {
            if (tblCells[j].innerHTML.toLowerCase().includes(toSearch.toLowerCase())) {
                containsSearch = true;
                break;
            }
        }
